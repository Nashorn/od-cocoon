(async (global)=>{ console.log("v5.0.2:04012022");
...
window.tag = function tag(target, name){
    ...
    console.warn(`@tag decorator on class '${target.prototype.namespace}' on is deprecated`);
    return;
}
...
namespace `core.ui` (
    class WebComponent extends HTMLElement {
        ...
        async loadTemplate() {
            return new Promise(async (resolve, reject) => {
                ...
                if(/\/*\.html$/.test(tem)){
                    ...
                    if(!this.__proto.template){
                        this.__proto.template = await imports(src, opts);
                    }
                    this._template = this.__proto.template
                }
        ...
        async render(data={}, t=this._template, outputEl) {
            ...
            if (t && typeof t=="string") {
                ...
                if (!this.inShadow() && this.isComposable()) {
                    ...
                    if(defaultSlot){
                        let nodes = Array.from(this.children);//Array.from(this.querySelectorAll("[slot]"));
                            // nodes = new Set([...nodes, Array.from(this.children).flat()])//nodes.length?nodes:Array.from(this.children)
                        for(let n of nodes){
        ...
        onAppendStyle(stylesheet) {
            if(this.inShadow()){
                try{
                    ...
                    //TODO: use code
                    console.warn(`${this.namespace}#onAppendStyle(): adoptedStyleSheets not yet supported. 
        ...
        async onTemplateLoaded() {
            // this.slots = !this.inShadow()&&Array.from(this.querySelectorAll("slot"))
            this.setClassList();
        ...
        //     //     this.slots : Array.from(this.children) 
        // }
        ...
        static defineAncestralClassList(){
            ...
            for(let ancestor of this.ancestors){
                ...
                if( proto['@cascade']||ancestor==this||this.prototype.ancestor==ancestor){
                    this.prototype.classes.unshift(ancestor.prototype.constructor.name);
                    if(this.prototype.ancestor==ancestor && !proto['@cascade']){break}
                } else { break }
            }
        }
        ...
        getNSStyleSheet(ns){
            var theme = Config.THEME ? `skins/${Config.THEME}/`:"";
            return relativeToAbsoluteFilePath(Config.SRC_PATH+`/./${theme}index.css`,ns);
        }

        setClassList(el=this.element||this) {
                el.className += " " + (this.__proto.classes.join(" ")).trim();
        ...
        async loadcss(urls) {
            ...
            return new Promise(async (resolve,reject) => {
                for(let ancestor of ancestors){
                    ...
                    if( ancestor != WebComponent && ancestor != Application && ancestor != World && ancestor != HTMLElement && ancestor.prototype.onLoadInstanceStylesheet() ){
                        var paths = [relativeToAbsoluteFilePath(Config.SRC_PATH+"/./index.css",ancestor.prototype.namespace)];
      ...
      if(Config.APP_WAITS_ON_SPLASH && !assetsloaded){
        ...
        let c = new system.http.ClassLoader;
            c.import(ns,path).then(async function init(){
              ...
              if((typeof World =="function" && app instanceof World) || app && app.isUpdatable) {
                window.world=app;
